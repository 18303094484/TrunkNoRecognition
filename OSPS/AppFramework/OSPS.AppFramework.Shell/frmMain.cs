...
namespace OSPS.AppFramework.Shell
{
    public partial class frmMain : Form
    {
        ...
        //private Thread _ContinuousShootThread = null;
        ...
        private bool _IsSmallMode = true;   
        public bool IsSmallMode
        {
            set { _IsSmallMode = value; }
            get { return _IsSmallMode; }
        }
        ...
        #region 手动抓拍车牌
        private void StartSnapTruckNoThread()
        {
            try
            {
                _SnapTruckNoThread = new Thread(new ThreadStart(SnapTruckNoThread));
                _SnapTruckNoThread.IsBackground = false;
                _SnapTruckNoThread.Name = "SnapTruckNoThread";
                _SnapTruckNoThread.Start();
            }
            catch (Exception ex)
            {
        ...
        private void SnapTruckNoThread()
        {
            SetTruckNoInvoke();
        }
        private delegate void SetTruckNoDelegate();
        private void SetTruckNoInvoke()
        {
            this.Invoke(new SetTruckNoDelegate(this.SetTruckNo));
        }
        //抓拍车牌
        private void SetTruckNo()
        {
            string jpgFilePath = "\\车牌图.jpg";
            if (File.Exists(jpgFilePath))
                File.Delete(jpgFilePath);

            string trunkNo = CaptrueTruckNo("10.80.79.33");
            ...
            {
                MessageBox.Show("未识别到车牌");
            }
        }
        /// <summary>
        /// 手动抓车牌
        /// </summary>
        /// <param name="szDVRIPAddress">摄像机IP</param>
        /// <returns></returns>
        private string CaptrueTruckNo(string szDVRIPAddress)
        {
            string strTruckNo = "";
            ...
            if (!OSPS.Device.HikSnap.CHCNetSDK.NET_DVR_ManualSnap(_AlarmHandle, ref struInter, ref struPlateResultInfo))
            {
            ...
            else
            {
                ...
                strTruckNo = struPlateResultInfo.struPlateInfo.sLicense;
                //if (strTruckNo.Substring(0, 1) == "黄")
                strTruckNo = strTruckNo.Substring(1);
        ...
        {
            btnTest.Visible = CommonVariable.IsAboveAdmin();
            lbAgent.Text = CommonVariable.CurrentUser.F_Name;
        }
        ...
        {
            //SendGateCtrlCommand();
            //StartSnapTruckNoThread();
            try
            {
                int state = OSPS.AppFramework.AccessDatabase.OrderSet.GetOrderStateByTrunkNo("鲁EA7615");
                MessageBox.Show("车票号状态为:" + state);
                if (state > 0)
                {
        ...
        {          
            Adam4060 adam = new Adam4060(_gateCtrlPort, 9600, 8, 0, 0);
        ...
        private bool StartPreview()
        {
            ...
            if (_RealHandle < 0)
            {
                ...
                lpPreviewInfo.hPlayWnd = this.pbPreview.Handle;//预览窗口
