...
{
    public class VCWincomSync
    {
        private int _ReadIntervalTimeout = 100;
        private int _ReadTotalTimeoutMultiplier = 50;
        ...
        {
            set { _ReadTotalTimeoutMultiplier = value; }
            get { return _ReadTotalTimeoutMultiplier; }
        }
        ...
        public VCWincomSync(string portNum, int baudRate, int nParity, int nDataBits, int nStopBits)
        {
            _PortNum = portNum;
            _BaudRate = baudRate;
            _DataBits = nDataBits;
        ...
        public void Open(int nTimeout)
        {
            ...
            // OPEN THE COMM PORT.
            string szCOMPort = "\\\\.\\" + _PortNum;
            hComm = CreateFile(szCOMPort, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
            ...
            {
                ctoCommPort.ReadIntervalTimeout = _ReadIntervalTimeout;
                ctoCommPort.ReadTotalTimeoutMultiplier = _ReadTotalTimeoutMultiplier;
            ...
            bool bRet = GetCommState(hComm, ref dcbCommPort);
            if (bRet)
            {
                string szTotalParity = "NOEM";
                string szParity = szTotalParity.Substring(_Parity, 1);
                string szDefine = String.Format("baud={0} parity={1} data={2} stop={3}", _BaudRate, szParity, _DataBits, _StopBits);
                bRet = BuildCommDCB(szDefine, ref dcbCommPort);
                if (bRet)
                {
                    dcbCommPort.Parity = Convert.ToByte(_Parity);
                    dcbCommPort.Bits1 = 0;
        ...
        public int Read(byte[] pBuffer, int nStart, int nLen)
        {
            ...
            if (hComm != INVALID_HANDLE_VALUE)
            {
                ...
                bool bRead = ReadFile(hComm, pBuffer, nLen, ref nReadLen, ref overLap);
                if (!bRead)
                {
                    _ErrorText = "Read Error!";
                }
            }
            else
            {
        ...
        public int Write(byte[] pBuffer, int nStart, int nLen)
        {
            ...
            {
                OVERLAPPED overLap = new OVERLAPPED();
                bool bWrite = WriteFile(hComm, pBuffer, nLen, ref BytesWritten, ref overLap);
                if (!bWrite)
                {
                    _ErrorText = "Write Error!";
                }
            }
            return BytesWritten;
        }
