...
Public Class Rs232 : Implements IDisposable
    ...
    Private miPort As String = "COM1"   '//  Default is COM1	
...
#Region "Win32API"
    '// Win32 API
    <DllImport("kernel32.dll", SetlastError:=True)> Private Shared Function GetFileSize(ByVal hFile As Long, ByVal lpFileSizeHigh As Long) As Int32
    End Function
...
#Region "Property"
    ...
    Public ReadOnly Property InbufferCount() As Int32
        Get
            Dim errN As Integer
            Dim comSt As New COMSTAT
            Dim ic As Integer
            ClearCommError(mhRS, errN, comSt)
            ic = comSt.cbInQue
            Return ic
        End Get
    End Property
...
#Region "Sub or Funcion"
    Public Overloads Sub Open()
        ...
        If miPort.Length > 0 Then
            Try
                '// Creates a COM Port stream handle 
                Dim filename As String
                filename = "\\.\" + miPort
                mhRS = CreateFile(filename, GENERIC_READ Or GENERIC_WRITE, 0, 0, OPEN_EXISTING, iMode, 0)
                If (mhRS.ToInt32 > 0) Then
                ...
                Else
                    ...
                    Dim sErrTxt As String = New Win32Exception().Message
                    Throw New CIOChannelException("Unable to open " + miPort + ControlChars.CrLf + sErrTxt)
                End If
        ...
        End If
    End Sub
    Public Overloads Sub Open(ByVal Port As String, ByVal BaudRate As Integer, ByVal DataBit As Integer, ByVal Parity As DataParity, ByVal StopBit As DataStopBit, ByVal BufferSize As Integer)
        '===================================================
        '
    ...
    Public Sub Close()
        ...
        End If
    End Sub
    Public Overloads Sub Write(ByVal Buffer As Byte(), ByVal nLen As Integer)
        '===================================================
        '
        ...
        Else
            ...
            Try
                ...
                iRc = WriteFile(mhRS, Buffer, nLen, 0, muOvlW)
                If iRc = 0 Then
                    ...
                    Else
                        ...
                        Else
                            '// All bytes sent?
                            If iBytesWritten <> nLen Then Throw New ApplicationException("Write Error - Bytes Written " & iBytesWritten.ToString & " of " & nLen.ToString)
                        End If
                    End If
                End If
            Finally
...
#Region "Private Routines"
    Private Sub pSetTimeout()
        ...
        Else
            ...
            With uCtm
                .ReadIntervalTimeout = 0    'miTimeout
                .ReadTotalTimeoutMultiplier = 0
                .ReadTotalTimeoutConstant = 0 'miTimeout
